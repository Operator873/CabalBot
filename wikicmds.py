...
def get_category_pages(bot, category):
    ...
    if len(data["query"]["categorymembers"]) == 0:
        ...
        bot.say(
            f"There are more than 10 pages listed. Please see {simple_wiki}{category.replace(' ', '_')}"
        )
    else:
...
def get_gb(bot, trigger):
    # !gb <ip>
    bot.say(
        "Global block: https://meta.wikimedia.org/wiki/Special:GlobalBlock/{}".format(
            trigger.group(3)
        )
    )
...
def google(bot, trigger):
    ...
    target = trigger.group(2)
    bot.say(
        "Google results: https://www.google.com/search?q=" + target.replace(" ", "+")
    )
